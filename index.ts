...
import { cancelUserRSVP } from "./src/fan/cancelUserRSVP";
...
app.post(cancelUserRSVPEndpoint, async (req, res) => {
  try {
    ...
    const response = await cancelUserRSVP(JWT, filmUID);

    console.log(
      "\x1b[32m",
      `Response from ${cancelUserRSVPEndpoint}: ${JSON.stringify(response)}`,
    );
    res.json(response);
  } catch (error) {
    console.error("\x1b[31m", `Error from ${cancelUserRSVPEndpoint}: ${error}`);
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : "Unknown Error",
    });
